...
import dashboard
from dashboard.widget import IconButton
from dashboard.widget.camera_calibration_window import ProcessWindow as CameraCalibrationWindow
from dashboard.widget.camera_window import InfoWindow as CameraInfoWindow
from dashboard.widget.camera_window import PreviewWindow as CameraPreviewWindow
from dashboard.widget.camera_window import SettingsWindow as CameraSettingsWindow
from dashboard.widget.capture_loop_window import ProcessWindow as CaptureLoopWindow
from dashboard.widget.dialog import MessageDialog
from dashboard.widget.dotf_measurement_window import ProcessWindow as DOTFMeasurementWindow
from dashboard.widget.modulator_window import InfoWindow as ModulatorInfoWindow
from dashboard.widget.modulator_window import PreviewWindow as ModulatorPreviewWindow
from dashboard.widget.modulator_window import SettingsWindow as ModulatorSettingsWindow
from dashboard.widget.pairwise_fpwfs_window import ProcessWindow as PairwiseFPWFSWindow
from dashboard.widget.recenter_window import ProcessWindow as RecenterWindow
from dashboard.widget.resource import ICON_EYE, ICON_GEAR, ICON_INFO, ICON_PAUSE, ICON_PLAY
from dashboard.widget.simple_loop_window import ProcessWindow as SimpleLoopWindow
from dashboard.widget.speckle_capture_window import ProcessWindow as SpeckleCaptureWindow
from dashboard.widget.speckle_nulling_window import ProcessWindow as SpeckleNullingWindow
from dashboard.worker.camera_worker import ProcessWorker as CameraSamplingWorker
from dashboard.worker.modulator_worker import ProcessWorker as ModulatorSamplingWorker
...
class MainWindow(QMainWindow):
    ...
    def open_device_preview_window(self, device: Camera | Modulator):
        ...
        def on_window_closed():
            dashboard.data.windows.pop(device.preview_window_id, None)

        if not dashboard.data.is_window_alive(device.preview_window_id):
            ...
            dashboard.data.windows[device.preview_window_id] = preview_window

            if dashboard.data.is_worker_alive(device.sampling_worker_id):
                dashboard.data.workers[device.sampling_worker_id].signals.sampled.connect(dashboard.data.windows[device.preview_window_id].on_sampled)

    def open_device_info_window(self, device: Camera | Modulator):

        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(device.info_window_id, None)

        if not dashboard.data.is_window_alive(device.info_window_id):
            ...
            dashboard.data.windows[device.info_window_id] = info_window

            if dashboard.data.is_worker_alive(device.sampling_worker_id):
                dashboard.data.workers[device.sampling_worker_id].signals.sampled.connect(dashboard.data.windows[device.info_window_id].on_sampled)

    def open_device_settings_window(self, device: Camera | Modulator):

        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(device.settings_window_id, None)

        if not dashboard.data.is_window_alive(device.settings_window_id):
            ...
            dashboard.data.windows[device.settings_window_id] = settings_window

    def on_play_pause(self, device: Camera | Modulator, button: IconButton):

        if dashboard.data.is_worker_alive(device.sampling_worker_id):  # an update worker is in progress
            device_sampling_worker = cast(CameraSamplingWorker | ModulatorSamplingWorker, dashboard.data.workers[device.sampling_worker_id])
        ...
        if dashboard.data.is_window_alive(device.preview_window_id):
            device_preview_window = cast(CameraPreviewWindow | ModulatorPreviewWindow, dashboard.data.windows[device.preview_window_id])
            device_sampling_worker.signals.sampled.connect(device_preview_window.on_sampled)
        if dashboard.data.is_window_alive(device.info_window_id):
            device_info_window = cast(CameraInfoWindow | ModulatorInfoWindow, dashboard.data.windows[device.info_window_id])
        ...
        dashboard.data.workers[device.sampling_worker_id] = device_sampling_worker

        dashboard.data.threadpool.start(device_sampling_worker)
    ...
    def on_sampling_worker_play(self, device: Camera | Modulator):
        if dashboard.data.is_window_alive(SimpleLoopWindow.wid):
            simple_loop_window = cast(SimpleLoopWindow, dashboard.data.windows[SimpleLoopWindow.wid])
            if simple_loop_window.source == device or simple_loop_window.sink == device:
                simple_loop_window.update_process_controls()
        if dashboard.data.is_window_alive(SpeckleCaptureWindow.wid):
            speckle_calibration_window = cast(SpeckleCaptureWindow, dashboard.data.windows[SpeckleCaptureWindow.wid])
        ...
        if dashboard.data.is_window_alive(SpeckleNullingWindow.wid):
            speckle_nulling_window = cast(SpeckleNullingWindow, dashboard.data.windows[SpeckleNullingWindow.wid])
        ...
        if dashboard.data.is_window_alive(RecenterWindow.wid):
            recenter_window = cast(RecenterWindow, dashboard.data.windows[RecenterWindow.wid])
            if recenter_window.source == device or recenter_window.sink == device:
                recenter_window.update_process_controls()
        if dashboard.data.is_window_alive(CameraCalibrationWindow.wid):
            camera_calibration_window = cast(CameraCalibrationWindow, dashboard.data.windows[CameraCalibrationWindow.wid])
            if camera_calibration_window.source == device:
                camera_calibration_window.update_process_controls()
        if dashboard.data.is_window_alive(DOTFMeasurementWindow.wid):
            dotf_measurement_window = cast(DOTFMeasurementWindow, dashboard.data.windows[DOTFMeasurementWindow.wid])
        ...
        if dashboard.data.is_window_alive(PairwiseFPWFSWindow.wid):
            pairwise_fpwfs_window = cast(PairwiseFPWFSWindow, dashboard.data.windows[PairwiseFPWFSWindow.wid])
    ...
    def on_sampling_worker_pause(self, device: Camera | Modulator, button: IconButton):
        if dashboard.data.is_worker_alive(device.sampling_worker_id):
            dashboard.data.workers.pop(device.sampling_worker_id)
    ...
    def on_add_device_clicked(self):
        ...
        for dialog_filename in dialog_filenames:
            ...
            if not dashboard.data.is_device_alive(stream_id):
                ...
                if stream.kind == Stream.Kind.CAMERA:
                    ...
                    dashboard.data.devices[stream_id] = camera
                    ...
                    dashboard.data.devices[stream_id] = modulator
    ...
    def on_remove_device_clicked(self):
        for index in sorted(set(i.row() for i in self.table.selectedIndexes()), reverse=True):
            ...
            dashboard.data.devices.pop(_key, None)
    ...
    def on_simple_loop_clicked(self):
        ...
        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(SimpleLoopWindow.wid, None)

        if not dashboard.data.is_window_alive(SimpleLoopWindow.wid):
            ...
            dashboard.data.windows[SimpleLoopWindow.wid] = simple_loop_window
    ...
    def on_capture_loop_clicked(self):
        ...
        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(CaptureLoopWindow.wid, None)

        if not dashboard.data.is_window_alive(CaptureLoopWindow.wid):
            ...
            dashboard.data.windows[CaptureLoopWindow.wid] = capture_window
    ...
    def on_speckle_capture_clicked(self):
        ...
        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(SpeckleCaptureWindow.wid, None)

        if not dashboard.data.is_window_alive(SpeckleCaptureWindow.wid):
            ...
            dashboard.data.windows[SpeckleCaptureWindow.wid] = speckle_capture_window
    ...
    def on_open_speckle_nulling_clicked(self):
        ...
        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(SpeckleNullingWindow.wid, None)

        if not dashboard.data.is_window_alive(SpeckleNullingWindow.wid):
            ...
            dashboard.data.windows[SpeckleNullingWindow.wid] = speckle_nulling_window
    ...
    def on_open_recenter_clicked(self):
        ...
        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(RecenterWindow.wid, None)

        if not dashboard.data.is_window_alive(RecenterWindow.wid):
            ...
            dashboard.data.windows[RecenterWindow.wid] = recenter_window
    ...
    def on_open_camera_calibration_clicked(self):
        ...
        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(CameraCalibrationWindow.wid, None)

        if not dashboard.data.is_window_alive(CameraCalibrationWindow.wid):
            ...
            dashboard.data.windows[CameraCalibrationWindow.wid] = camera_calibration_window
    ...
    def on_open_dotf_measurement_clicked(self):
        ...
        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(DOTFMeasurementWindow.wid, None)

        if not dashboard.data.is_window_alive(DOTFMeasurementWindow.wid):
            ...
            dashboard.data.windows[DOTFMeasurementWindow.wid] = dotf_measurement_window
    ...
    def on_open_pairwise_fpwfs_clicked(self):
        ...
        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(PairwiseFPWFSWindow.wid, None)

        if not dashboard.data.is_window_alive(PairwiseFPWFSWindow.wid):
            ...
            dashboard.data.windows[PairwiseFPWFSWindow.wid] = pairwise_fpwfs_window

    def closeEvent(self, event):
        if dashboard.data.windows:
            logger.info("Cannot close main window until all child windows are closed.")
            event.ignore()
        else:
            while dashboard.data.workers:
                key, worker = dashboard.data.workers.popitem()
                worker.stop()
