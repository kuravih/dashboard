...
from pytestbed import DOTFProbeDirection
...
logger = setup_logger(f"{dashboard.DOTF_MEASUREMENT}_window", terminator="\n")
...
class ProcessInfoWindow(Window):
    ...
    wid = f"{dashboard.DOTF_MEASUREMENT}_info_window"
...
class ProcessWindow(Window):
    ...
    wid = f"{dashboard.DOTF_MEASUREMENT}_window"
    ...
    def open_process_info_clicked(self):
        ...
        if not dashboard.data.is_window_alive(ProcessInfoWindow.wid) and self.source is not None and self.sink is not None:
            ...
            if ProcessInfoWindow.wid in dashboard.data.workers:
                process_worker = cast(ProcessWorker, dashboard.data.workers[ProcessInfoWindow.wid])
                process_worker.signals.dotfMeasured.connect(process_info_window.on_dotf_measured)
    ...
    def setup_main_widget(self) -> QWidget:
        ...
        def probe_direction_changed(values: list[DOTFProbeDirection]):
            if ProcessInfoWindow.wid in dashboard.data.windows:
                dashboard.data.windows.pop(ProcessInfoWindow.wid).close()
