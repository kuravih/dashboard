...
logger = setup_logger(f"{dashboard.CAMERA_CALIBRATION}_window", terminator="\n")
...
class ProcessWindow(Window):
    ...
    wid = f"{dashboard.CAMERA_CALIBRATION}_window"
    ...
    def update_process_controls(self):
        enabled = False
        if self.source is not None:
            if not dashboard.data.is_worker_alive(self.source.sampling_worker_id):
                enabled = True
    ...
    def on_start_stop_clicked(self):
        if self.source is None:
            ...
            source_storage_worker = SourceStorageWorker(f"data/output/{timestamp}_{dashboard.CAMERA_CALIBRATION}_{self.source.name}_dark.raw", self.settings_widget.exposure_times_array.size)
    ...
    def setup_main_widget(self) -> QWidget:
        ...
        self.device_widget = DevicesSetupWidget(dashboard.data.devices, setup_sink=False, parent=self)
