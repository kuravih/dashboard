...
from pytestbed import Flip, Rotation
from pytestbed.function import flip_rotate_frame, is_modulator_calibration_file_valid

import dashboard

from pytestbed.device.modulator import Modulator, SinkSample
...
class SettingsWindow(Window):
    ...
    def setup_settings_widget(self):
        ...
        def on_set_radius_clicked(radius: int):
            ...
            if dashboard.data.is_window_alive(self.modulator.preview_window_id):
                modulator_preview_window = cast(PreviewWindow, dashboard.data.windows[self.modulator.preview_window_id])
                modulator_preview_window.preview_figure_widget.figure.get_imshow_axes().get_mask().set_alpha(mask.astype(np.float64))
            if dashboard.data.is_window_alive(self.modulator.presets_window_id):
                modulator_presets_window = cast(PresetsWindow, dashboard.data.windows[self.modulator.presets_window_id])
        ...
        def on_presets_clicked():
            ...
            def on_window_closed():
                dashboard.data.windows.pop(self.modulator.presets_window_id, None)

            if not dashboard.data.is_window_alive(self.modulator.presets_window_id):
                ...
                dashboard.data.windows[self.modulator.presets_window_id] = presets_window
