...
from pytestbed import PairwiseProbeDirection
from pytestbed.function import is_camera_calibration_file_valid, is_pairwise_calibration_file_valid, read_camera_calibration_file, read_pairwise_calibration_file
...
logger = setup_logger(f"{dashboard.PAIRWISE_FPWFS}_window", terminator="\n")
...
class ProcessSettingsWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._probe_amplitude_perc_spinbox = QDoubleSpinBox(self)
        self._probe_amplitude_perc_spinbox.setRange(_probe_amplitude_perc_min, _probe_amplitude_perc_max)
        self._probe_amplitude_perc_spinbox.setSuffix(" %")
        self._probe_amplitude_perc_spinbox.setSingleStep(1)
        self._probe_amplitude_perc_spinbox.setToolTip("Probe amplitude")
        self._probe_amplitude_perc_spinbox.setValue(_probe_amplitude_perc)
        self._probe_amplitude_perc_spinbox.setEnabled(False)

        probe_dξ_label = QLabel("Probe dξ", self)
        probe_dξ_label.setFixedWidth(150)

        self._probe_dξ_spinbox = QDoubleSpinBox(self)
        self._probe_dξ_spinbox.setRange(0, 0.02000)
        self._probe_dξ_spinbox.setSingleStep(0.00001)
        self._probe_dξ_spinbox.setDecimals(5)
        self._probe_dξ_spinbox.setToolTip("Probe dξ")
        self._probe_dξ_spinbox.setValue(0)
        self._probe_dξ_spinbox.setEnabled(False)

        probe_dη_label = QLabel("Probe dη", self)
        probe_dη_label.setFixedWidth(150)

        self._probe_dη_spinbox = QDoubleSpinBox(self)
        self._probe_dη_spinbox.setRange(0, 0.0500)
        self._probe_dη_spinbox.setSingleStep(0.0001)
        self._probe_dη_spinbox.setDecimals(4)
        self._probe_dη_spinbox.setToolTip("Probe dη")
        self._probe_dη_spinbox.setValue(0)
        self._probe_dη_spinbox.setEnabled(False)

        probe_ξc_label = QLabel("Probe ξc", self)
        probe_ξc_label.setFixedWidth(150)

        self._probe_ξc_spinbox = QDoubleSpinBox(self)
        self._probe_ξc_spinbox.setRange(0.0, 0.0002000000000)
        self._probe_ξc_spinbox.setSingleStep(0.0000000000001)
        self._probe_ξc_spinbox.setToolTip("Probe ξc")
        self._probe_ξc_spinbox.setDecimals(13)
        self._probe_ξc_spinbox.setValue(0)
        self._probe_ξc_spinbox.setEnabled(False)
        ...
        star_brightness_label = QLabel("Star Brightness", self)
        star_brightness_label.setFixedWidth(150)

        self.star_brightness_spinbox = QDoubleSpinBox(self)
        self.star_brightness_spinbox.setRange(0, 2**16 - 1)
        self.star_brightness_spinbox.setSingleStep(0.0001)
        self.star_brightness_spinbox.setToolTip("Star brightness")
        self.star_brightness_spinbox.setSuffix(" adu")
        self.star_brightness_spinbox.setDecimals(4)
        self.star_brightness_spinbox.setValue(51234.8080)
        ...
        col += 1
        widget_layout.addWidget(self._probe_amplitude_perc_spinbox, row, col, 1, 3)

        row += 1
        col = 0
        widget_layout.addWidget(probe_dξ_label, row, col)
        col += 1
        widget_layout.addWidget(self._probe_dξ_spinbox, row, col, 1, 3)

        row += 1
        col = 0
        widget_layout.addWidget(probe_dη_label, row, col)
        col += 1
        widget_layout.addWidget(self._probe_dη_spinbox, row, col, 1, 3)

        row += 1
        col = 0
        widget_layout.addWidget(probe_ξc_label, row, col)
        col += 1
        widget_layout.addWidget(self._probe_ξc_spinbox, row, col, 1, 3)
        ...
        row += 1
        col = 0
        widget_layout.addWidget(star_brightness_label, row, col)
        col += 1
        widget_layout.addWidget(self.star_brightness_spinbox, row, col)
    ...
    def probe_amplitude_perc(self) -> float:
        return self._probe_amplitude_perc_spinbox.value() / 100.0

    @probe_amplitude_perc.setter
    def probe_amplitude_perc(self, value: float):
        self._probe_amplitude_perc_spinbox.setValue(value * 100.0)

    @property
    def probe_ξc(self) -> float:
        return self._probe_ξc_spinbox.value()

    @probe_ξc.setter
    def probe_ξc(self, value: float):
        self._probe_ξc_spinbox.setValue(value)

    @property
    def probe_dξ(self) -> float:
        return self._probe_dξ_spinbox.value()

    @probe_dξ.setter
    def probe_dξ(self, value: float):
        self._probe_dξ_spinbox.setValue(value)

    @property
    def probe_dη(self) -> float:
        return self._probe_dη_spinbox.value()

    @probe_dη.setter
    def probe_dη(self, value: float):
        return self._probe_dη_spinbox.setValue(value)
    ...
    @property
    def star_brightness_experiment(self) -> float:
        return self.star_brightness_spinbox.value()
    ...
    def normalization(self) -> float:
        return np.sqrt(self.star_brightness_model / self.star_brightness_experiment)
...
class ProcessInfoWindow(Window):
    ...
    wid = f"{dashboard.PAIRWISE_FPWFS}_info_window"
...
class ProcessWindow(Window):
    ...
    wid = f"{dashboard.PAIRWISE_FPWFS}_window"
    ...
    def update_process_controls(self):
        ...
        if self.source is not None and self.sink is not None and self.settings_widget.pairwise_calibration_widget.filepath is not None:
            if not dashboard.data.is_worker_alive(self.source.sampling_worker_id) and not dashboard.data.is_worker_alive(self.sink.sampling_worker_id):
                enabled = True
    ...
    def on_pairwise_calibration_change(self):
        if self.source is not None and self.sink is not None and self.settings_widget.pairwise_calibration_widget.filepath is not None:
            self.settings_widget.star_brightness_model, self.settings_widget.pairwise_calibration, probe_amp, probe_ξc, probe_dξ, probe_dη = read_pairwise_calibration_file(self.settings_widget.pairwise_calibration_widget.filepath)
            self.settings_widget.probe_amplitude_perc = probe_amp / 10e-9
            self.settings_widget.probe_ξc = probe_ξc
            self.settings_widget.probe_dξ = probe_dξ
            self.settings_widget.probe_dη = probe_dη
            self.controls_widget.info_button.setEnabled(True)
            self.controls_widget.run_stop_button.setEnabled(True)
        else:
            self.settings_widget.star_brightness_model, self.settings_widget.pairwise_calibration = None, None
            self.settings_widget.probe_amplitude_perc = 0.0
            self.settings_widget.probe_ξc = 0.0
            self.settings_widget.probe_dξ = 0.0
            self.settings_widget.probe_dη = 0.0
    ...
    def on_camera_calibration_change(self):
        ...
        if self.source is not None and self.settings_widget.camera_calibration_widget.filepath is not None:
            self.settings_widget.camera_calibration = read_camera_calibration_file(self.settings_widget.camera_calibration_widget.filepath, self.source.roi)
            self.controls_widget.run_stop_button.setEnabled(True)
    ...
    def on_source_changed(self, device: Camera):
        ...
        if dashboard.data.is_window_alive(ProcessInfoWindow.wid):
            process_info_window = cast(ProcessInfoWindow, dashboard.data.windows.pop(ProcessInfoWindow.wid))
    ...
    def on_process_finished(self):
        ...
        self.controls_widget.progressbar.updateProgress()
        if dashboard.data.is_worker_alive(ProcessWorker.wid):
            dashboard.data.workers.pop(ProcessWorker.wid)
    ...
    def on_start_stop_clicked(self):
        if self.source is None or self.sink is None:
            ...
            if dashboard.data.is_worker_alive(ProcessWorker.wid):
                process_worker = cast(ProcessWorker, dashboard.data.workers[ProcessWorker.wid])
                process_worker.stop()
                return

            process_worker = ProcessWorker(self.source, self.sink, self.settings_widget.dark_hole_mask, self.settings_widget.normalization, self.settings_widget.camera_calibration, self.settings_widget.pairwise_calibration, (self.settings_widget.probe_amplitude_perc, self.settings_widget.probe_dξ, self.settings_widget.probe_dη, self.settings_widget.probe_ξc, self.settings_widget.probe_directions), self.settings_widget.n_reps)
            ...
            if dashboard.data.is_window_alive(self.source.preview_window_id):
                source_preview_window = cast(CameraPreviewWindow, dashboard.data.windows[self.source.preview_window_id])
                process_worker.signals.srcSampled.connect(source_preview_window.on_sampled)

            if dashboard.data.is_window_alive(self.sink.preview_window_id):
                sink_preview_window = cast(ModulatorPreviewWindow, dashboard.data.windows[self.sink.preview_window_id])
                process_worker.signals.snkSampled.connect(sink_preview_window.on_sampled)

            if dashboard.data.is_window_alive(ProcessInfoWindow.wid):
                process_info_window = cast(ProcessInfoWindow, dashboard.data.windows[ProcessInfoWindow.wid])
                process_worker.signals.wfSensed.connect(process_info_window.on_wf_sensed)

            dashboard.data.workers[ProcessWorker.wid] = process_worker
            dashboard.data.threadpool.start(process_worker)
            self.update_device_buttons(False)

    def open_process_info_clicked(self):
        @Slot()
        def on_window_closed():
            dashboard.data.windows.pop(ProcessInfoWindow.wid, None)

        if not dashboard.data.is_window_alive(ProcessInfoWindow.wid) and self.source is not None and self.sink is not None:
            ...
            dashboard.data.windows[ProcessInfoWindow.wid] = process_info_window
    ...
    def setup_main_widget(self) -> QWidget:
        ...
        self.devices_widget = DevicesSetupWidget(dashboard.data.devices, parent=self)
