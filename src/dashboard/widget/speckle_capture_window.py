...
from pytestbed.function import flip_rotate_points
...
from ..function import speckle_preset_validator
...
logger = setup_logger(f"{dashboard.SPECKLE_CAPTURE}_window", terminator="\n")
...
class ProcessSettingsWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.angle_deg_steps_linspace.start_spinbox.setSuffix(" °")
        self.angle_deg_steps_linspace.stop_spinbox.setMinimumWidth(100)
        self.angle_deg_steps_linspace.stop_spinbox.setSuffix(" °")
        ...
        self.phase_deg_steps_linspace.start_spinbox.setSuffix(" °")
        ...
        def on_load_preset_clicked():
            dialog_filename, _ = QFileDialog.getOpenFileName(self, "Load Speckle Preset", "./data/output", "toml file (*.toml)", options=QFileDialog.Option.DontUseNativeDialog | QFileDialog.Option.ReadOnly)
            if dialog_filename:
                file_info = QFileInfo(dialog_filename)
                filepath = f"{file_info.absolutePath()}/{file_info.fileName()}"
                data = speckle_preset_validator(filepath)
                if data is not None:
                    self.amplitude_perc_spinbox.setValue(float(data["amplitude"]["perc"]))
                    self.angle_deg_steps_linspace.start_spinbox.setValue(float(data["angle"]["start_deg"]))
                    self.angle_deg_steps_linspace.stop_spinbox.setValue(float(data["angle"]["stop_deg"]))
                    self.angle_deg_steps_linspace.num_spinbox.setValue(int(data["angle"]["steps"]))
                    self.frequency_steps_linspace.start_spinbox.setValue(float(data["frequency"]["start"]))
                    self.frequency_steps_linspace.stop_spinbox.setValue(float(data["frequency"]["stop"]))
                    self.frequency_steps_linspace.num_spinbox.setValue(int(data["frequency"]["steps"]))
        ...
        self.load_preset_button = IconButton(QIcon(ICON_FOLDER_OPEN), parent=self)
        self.load_preset_button.setToolTip("Load preset")
        self.load_preset_button.clicked.connect(on_load_preset_clicked)

        @Slot()
        def on_save_preset_clicked():
            dialog_filename, _ = QFileDialog.getSaveFileName(self, "Save Speckle Preset", "./data/output", "toml file (*.toml)", options=QFileDialog.Option.DontUseNativeDialog)
            if dialog_filename:
                if not dialog_filename.endswith(".toml"):
                    dialog_filename += ".toml"
                preset = {
                    "amplitude": {"perc": self.amplitude_perc_spinbox.value()},
                ...
                try:
                    ...
                    MessageDialog("Save Failed", str(e), icon=QMessageBox.Icon.Critical, buttons=QMessageBox.StandardButton.Ok).exec()

        self.save_preset_button = IconButton(QIcon(ICON_DISK), parent=self)
        self.save_preset_button.setToolTip("Save preset")
        self.save_preset_button.clicked.connect(on_save_preset_clicked)
        ...
        row += 1
        col = 0
        widget_layout.addWidget(presets_label, row, col)
        col += 1
        widget_layout.addWidget(self.load_preset_button, row, col)
        col += 1
        widget_layout.addWidget(self.save_preset_button, row, col)
...
class ProcessWindow(Window):
    ...
    wid = f"{dashboard.SPECKLE_CAPTURE}_window"
    ...
    def on_source_storage_finished(self):
        ...
        if dashboard.data.is_worker_alive(self.source.storage_worker_id):
            dashboard.data.workers.pop(self.source.storage_worker_id)

    @Slot()
    def on_sink_storage_finished(self):
        assert self.sink is not None
        if dashboard.data.is_worker_alive(self.sink.storage_worker_id):
            dashboard.data.workers.pop(self.sink.storage_worker_id)
    ...
    def on_start_stop_clicked(self):
        if self.source is None or self.sink is None:
            ...
            if dashboard.data.is_worker_alive(self.source.storage_worker_id):
                source_storage_worker = cast(SourceStorageWorker, dashboard.data.workers[self.source.storage_worker_id])
                source_storage_worker.stop()

            if dashboard.data.is_worker_alive(self.sink.storage_worker_id):
                sink_storage_worker = cast(SinkStorageWorker, dashboard.data.workers[self.sink.storage_worker_id])
            ...
            process_worker = ProcessWorker(self.source, self.sink, self.settings_widget.amplitude_perc, self.settings_widget.frequency_array, self.settings_widget.angle_rad_array, self.settings_widget.phase_rad_array, self.settings_widget.n_reps)
            ...
            with open(f"data/output/{timestamp}_{dashboard.SPECKLE_CAPTURE}_variables.pkl", "wb") as wbfile:
            ...
            source_storage_worker = SourceStorageWorker(f"data/output/{timestamp}_{dashboard.SPECKLE_CAPTURE}_{self.source.name}.raw", process_worker.n_ticks)
            ...
            dashboard.data.workers[self.source.storage_worker_id] = source_storage_worker
            dashboard.data.threadpool.start(source_storage_worker)

            sink_storage_worker = SinkStorageWorker(f"data/output/{timestamp}_{dashboard.SPECKLE_CAPTURE}_{self.sink.name}.raw", process_worker.n_ticks)
            ...
            dashboard.data.workers[self.sink.storage_worker_id] = sink_storage_worker
            dashboard.data.threadpool.start(sink_storage_worker)
    ...
    def closeEvent(self, event):
        ...
        if dashboard.data.is_worker_alive(ProcessWorker.wid):
            process_worker = cast(ProcessWorker, dashboard.data.workers[ProcessWorker.wid])
            process_worker.stop()
        if self.source is not None and dashboard.data.is_worker_alive(self.source.storage_worker_id):
            source_storage_worker = cast(SourceStorageWorker, dashboard.data.workers[self.source.storage_worker_id])
            source_storage_worker.stop()
        if self.sink is not None and dashboard.data.is_worker_alive(self.sink.storage_worker_id):
            sink_storage_worker = cast(SinkStorageWorker, dashboard.data.workers[self.sink.storage_worker_id])
            sink_storage_worker.stop()
        self.deleteLater()
        event.accept()
