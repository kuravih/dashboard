...
from pytestbed.function import constrained_sin_fit_fn, is_speckle_calibration_file_valid, quadratic_fit_fn, read_speckle_calibration_file
...
logger = setup_logger(f"{dashboard.SPECKLE_NULLING}_window", terminator="\n")
...
class ProcessInfoWindow(Window):
    ...
    wid = f"{dashboard.SPECKLE_NULLING}_info_window"
...
class ProcessPreviewWindow(Window):
    ...
    wid = f"{dashboard.SPECKLE_NULLING}_preview_window"
...
class ProcessWindow(Window):
    ...
    wid = f"{dashboard.SPECKLE_NULLING}_window"
    ...
    def on_source_changed(self, device: Camera):
        ...
        if dashboard.data.is_window_alive(ProcessPreviewWindow.wid):
            process_preview_window = cast(ProcessPreviewWindow, dashboard.data.windows.pop(ProcessPreviewWindow.wid))
    ...
    def on_start_stop_clicked(self):
        if self.source is None or self.sink is None:
            ...
            if dashboard.data.is_window_alive(ProcessPreviewWindow.wid):
                process_preview_window = cast(ProcessPreviewWindow, dashboard.data.windows[ProcessPreviewWindow.wid])
    ...
    def open_process_preview_clicked(self):
        ...
        def on_window_closed():
            dashboard.data.windows.pop(ProcessPreviewWindow.wid, None)

        if not dashboard.data.is_window_alive(ProcessPreviewWindow.wid) and self.source is not None and self.sink is not None:
            ...
            dashboard.data.windows[ProcessPreviewWindow.wid] = process_preview_window
