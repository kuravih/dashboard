...
from pytestbed.function import PairwiseProbeDirection, calculate_quantum_efficiency, capture_to_intensity, pairwise_estimate, pairwise_estimation_matrices, pairwise_probe
...
logger = setup_logger(f"{dashboard.PAIRWISE_FPWFS}_worker", terminator="\n")
...
class ProcessWorker(Worker):
    wid = f"{dashboard.PAIRWISE_FPWFS}_worker"

    _allowed_slots_ = Worker._allowed_slots_ | {"signals", "source", "sink", "camera_calibration", "normalization", "qe_perc", "probe_amplitude_perc", "dark_hole_mask", "iCAψ", "probe_dξ", "probe_dη", "probe_ξc", "probe_directions", "wavefront", "n_reps"}

    def __init__(self, source: Camera, sink: Modulator, dark_hole_mask: NDArray[np.bool], normalization: float, camera_calibration: dict[str, np.ndarray | float | int], pairwise_calibration: dict[int, NDArray[np.complex128]], probe: tuple[float, float, float, float, list[PairwiseProbeDirection]], n_reps: int):
        ...
        self.probe_amplitude_perc, self.probe_dξ, self.probe_dη, self.probe_ξc, self.probe_directions = probe
        self.wavefront = {}
    ...
    def sense_wavefront(self, iCAψ: dict[int, NDArray[np.complex128]], probe_amplitude: float, dξ: float, dη: float, ξc: float, direction: PairwiseProbeDirection) -> NDArray[np.complex128]:
        ...
        electric_field[self.dark_hole_mask] = self.normalization * pairwise_estimate(probe_p1_intensity[self.dark_hole_mask], probe_m1_intensity[self.dark_hole_mask], probe_p2_intensity[self.dark_hole_mask], probe_m2_intensity[self.dark_hole_mask], pairwise_estimation_matrices(iCAψ1[self.dark_hole_mask], iCAψ2[self.dark_hole_mask]))
