...
from pytestbed import DOTFProbeDirection, SinkSample, SourceSample
from pytestbed.function import dotf_probe, image_shift, locate_single_airy, locate_single_airy_with_radius
...
logger = setup_logger(f"{dashboard.DOTF_MEASUREMENT}_worker", terminator="\n")
...
class ProcessWorker(Worker):
    wid = f"{dashboard.DOTF_MEASUREMENT}_worker"
    ...
    def __init__(self, source: Camera, sink: Modulator, probe: tuple[float, tuple[int, int], list[DOTFProbeDirection]], n_reps: int = 0):
        self.signals = ProcessWorkerSignals()
        self.source = source
        self.sink = sink
        self.probe_amplitude_perc, self.probe_size, self.probe_directions = probe
